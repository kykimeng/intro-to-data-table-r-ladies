...
a. Import `mpg` dataset. To do this you need to load `ggplot2` library: `library(ggplot2)`. If you don't have this library installed, you can do so by executing `install.packages("ggplot2")`. Once `ggplot2` is loaded, import the data: `data("mpg")`. 
...
c. Calculate number of observations by `manufacturer`
...
Use `mpg` dataset:
a. Set `manufacturer`, `class`, and `model` as keys
b. Calculate maximum engine displacement (*displ*) by class and model, using `keyby`
...
a. Create `drv_description` data.table using this code: `drv_description <- data.table(drv = c("f", "r", "4"), drv_desc = c("front-wheel drive", "reer-wheel drive", "four-wheel drive"), key = "drv")`
b. Join `mpg` and `drv_description` data.tables on `drv` variable using `DT1[DT2]` syntax
c. Join `mpg` and `drv_description` data.tables on `drv` variable using `merge()` function
...
Use `mpg` dataset:
a. Create a `manual_dummy`, an indicator whether the transmission is manual (*hint: `grepl("manual", trans)`*)
b. Repeat a. but this time also create a `row_id` using `.I` all in one go.
c. Let's assume that `displ` was entered incorrectly. Use sub-assign by reference to update `displ` with value of less than 2 to equal 2. 
